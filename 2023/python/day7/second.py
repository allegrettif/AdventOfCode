...
SYMBOL_ORDER = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
...
def compare(a: tuple[str, int, str, str], b: tuple[str, int, str, str]) -> int:
    a_hand, a_point, a_type, a_highest, a_dict = a
    b_hand, b_point, b_type, b_highest, b_dict = b
    if a_type == b_type:
...
def convert_to_hands(file_content: list[str]) -> list[tuple[str, int, str, str, dict[str, int]]]:
    ...
    for line in file_content:
        ...
        hands.append((hand, int(points), hand_type, highest_symbol(hand), chars))
